...
export default async function Home() {
  ...
  return <main className="readonly-board" data-instrument={technique}>
    ...
    <section className="readonly-grid" aria-label={`${techniqueLabel} 样品测试阅览`}>
      ...
      <article className={`readonly-panel readonly-device-panel ${technique}`}><header className="readonly-panel-header"><div><span className={`readonly-kicker ${technique}`}>{techniqueLabel}</span><h2>设备状态</h2></div></header><InstrumentReadout instrument={instrument} sample={activeSample} /></article>
