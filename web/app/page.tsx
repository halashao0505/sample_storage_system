...
export default function Home() {
  ...
  return <div className="workspace-shell">
    ...
    <main className="workspace-content"><section className="page-heading"><div><p>LAB OPERATIONS · ONE PAGE WORKSPACE</p><h1>样品测试运营工作台</h1><span>统一查看 XAFS、XRD 样品、设备、队列、记录与趋势。</span></div><div className="scope-switch" aria-label="谱仪范围">{(['all', 'xafs', 'xrd'] as InstrumentScope[]).map((item) => <button key={item} className={scope === item ? 'active' : ''} type="button" onClick={() => setScope(item)}><ScopeMark scope={item} /> {item === 'all' ? '全部谱仪' : item.toUpperCase()}</button>)}</div></section>
