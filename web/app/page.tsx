...
function WorkspacePanel({ id, title, subtitle, className = '', children, draggingId, onDragStart, onDragOver, onDrop }: {
  ...
  return <article className={`workspace-panel ${className} ${draggingId === id ? 'is-dragging' : ''}`} onDragOver={onDragOver} onDrop={(event) => onDrop(event, id)}>
    <header className="panel-bar" draggable onDragStart={(event) => onDragStart(event, id)}><span className="panel-grip" aria-hidden="true">⠿</span><div><h2>{title}</h2><p>{subtitle}</p></div><span className="panel-resize-note">拖动排序 · 右下角调大小</span></header>
