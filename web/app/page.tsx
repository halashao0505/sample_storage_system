...
import { ReadOnlyTrendChart } from '../components/read-only/trend-chart';
...
export default async function Home() {
  ...
  return <main className="readonly-board" data-instrument={technique}>
    <section className="readonly-grid" aria-label={`${techniqueLabel} 样品测试阅览`}>
      <article className="readonly-panel readonly-trend-panel"><header className="readonly-panel-header"><div><span className={`readonly-kicker ${technique}`}>{techniqueLabel}</span><h1>测试趋势</h1></div><span className="readonly-update">数据同步 · {dashboardSnapshot.syncedAt}</span></header><ReadOnlyTrendChart values={trend} technique={technique} metric="samples" /></article>
