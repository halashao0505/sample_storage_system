...
export function ReadOnlyTrendChart({ values, technique, metric }: ReadOnlyTrendChartProps) {
  ...
  return <div className="readonly-chart-wrap">
    <div className="readonly-chart-meta"><span>{metricLabels[metric]} · 今日</span><strong>{values.at(-1) ?? 0}</strong></div>
    <svg className="readonly-line-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${technique.toUpperCase()} ${metricLabels[metric]}趋势图`}>
      <defs><linearGradient id={`area-${technique}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={accent} stopOpacity=".24" /><stop offset="100%" stopColor={accent} stopOpacity="0" /></linearGradient></defs>
