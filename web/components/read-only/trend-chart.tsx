...
export function ReadOnlyTrendChart({ values, technique, metric, rangeLabel }: ReadOnlyTrendChartProps) {
  ...
  return <div className="readonly-chart-wrap">
    ...
    <svg className="readonly-line-chart" viewBox={`0 0 ${width} ${height}`} role="img" aria-label={`${technique.toUpperCase()} ${metricLabels[metric]}趋势图`}>
      <defs><linearGradient id={`area-${technique}`} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={accent} stopOpacity=".24" /><stop offset="100%" stopColor={accent} stopOpacity="0" /></linearGradient><linearGradient id={`wave-${technique}`} gradientUnits="userSpaceOnUse" x1="0" y1="0" x2="170" y2="0"><stop offset="0%" stopColor={accent} stopOpacity="0" /><stop offset="35%" stopColor={accent} stopOpacity="0" /><stop offset="49%" stopColor="#f7fbff" stopOpacity=".98" /><stop offset="54%" stopColor={technique === 'xafs' ? '#d9cfff' : '#c6f2f7'} stopOpacity=".98" /><stop offset="68%" stopColor={accent} stopOpacity="0" /><stop offset="100%" stopColor={accent} stopOpacity="0" /><animateTransform attributeName="gradientTransform" type="translate" from="-170 0" to="760 0" dur="3.8s" repeatCount="indefinite" /></linearGradient></defs>
