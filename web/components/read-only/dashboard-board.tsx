...
export function DashboardBoard({ technique, initialSnapshot }: { technique: SampleTechnique; initialSnapshot: TechniqueSnapshot }) {
  ...
  return <main className={`readonly-board communication-${communication.state}`} data-instrument={technique}>
    ...
    <section className="readonly-grid" aria-label={`${label} 样品测试阅览`}>
      ...
      <article className="readonly-panel readonly-queue-panel"><header className="readonly-panel-header"><h2>待测试队列</h2></header><ol className="readonly-queue-list">{visibleQueue.map((item) => <li key={item.sampleId}><span className="queue-index">{String(item.position).padStart(2, '0')}</span><div><strong>{item.sampleId}</strong><small>{item.sampleName} · 预计 {item.scheduledAt}</small></div><span className="queue-duration">{item.estimate}</span></li>)}</ol><div className="queue-finish"><span>队列状态</span><strong>{communication.state === 'connected' ? '按实时数据更新' : '等待通讯恢复'}</strong></div></article>
    </section>
  </main>;
}
