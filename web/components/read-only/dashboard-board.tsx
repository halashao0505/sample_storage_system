...
import type { CommunicationStatus, SampleTechnique, TechniqueSnapshot } from '../../lib/samples/types';
...
export function DashboardBoard({ technique, initialSnapshot }: { technique: SampleTechnique; initialSnapshot: TechniqueSnapshot }) {
  ...
  return <main className={`readonly-board communication-${communication.state}`} data-instrument={technique}>
    ...
    <section className="readonly-grid" aria-label={`${label} 样品测试阅览`}>
      <article className="readonly-panel readonly-trend-panel"><KeyboardTrend technique={technique} values={snapshot.trends} communication={communication} /></article>
      <article className={`readonly-panel readonly-device-panel ${technique}`}><header className="readonly-panel-header"><div><span className={`readonly-kicker ${technique}`}>{label}</span><h2>设备状态</h2></div></header><InstrumentReadout instrument={instrument} sample={activeSample} communication={communication} /></article>
      <article className="readonly-panel readonly-records-panel"><header className="readonly-panel-header"><div><span className={`readonly-kicker ${technique}`}>{label}</span><h2>样品记录</h2></div><span className="readonly-count">{snapshot.records.length} 条</span></header><div className="table-scroll"><table><thead><tr><th>样品</th><th>申请人</th><th>设备</th><th>开始</th><th>耗时</th><th>状态</th></tr></thead><tbody>{snapshot.records.map((sample) => <tr key={sample.id}><td><strong>{sample.id}</strong><span>{sample.name}</span></td><td>{sample.applicant}</td><td>{sample.instrument}</td><td>{sample.startedAt}</td><td>{sample.duration}</td><td><span className={`state-tag ${sample.state}`}>{sample.state === 'running' ? '测试中' : sample.state === 'queued' ? '待测试' : sample.state === 'completed' ? '已完成' : '需复测'}</span></td></tr>)}</tbody></table></div></article>
      <article className="readonly-panel readonly-queue-panel"><header className="readonly-panel-header"><div><span className={`readonly-kicker ${technique}`}>{label}</span><h2>待测试队列</h2></div><span className="readonly-count">{snapshot.queue.length} 项</span></header><ol className="readonly-queue-list">{snapshot.queue.map((item) => <li key={item.sampleId}><span className="queue-index">{String(item.position).padStart(2, '0')}</span><div><strong>{item.sampleId}</strong><small>{item.sampleName} · 预计 {item.scheduledAt}</small></div><span className="queue-duration">{item.estimate}</span></li>)}</ol><div className="queue-finish"><span>队列状态</span><strong>{communication.state === 'connected' ? '按实时数据更新' : '等待通讯恢复'}</strong></div></article>
    </section>
  </main>;
}
